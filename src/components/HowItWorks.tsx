    ...
    >
      ...
      <ScrollReveal>
        <div className="flex flex-col items-center text-center mb-12 sm:mb-16">
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 mt-8 p-1.5 rounded-full bg-white/[0.03] border border-white/10 backdrop-blur-md max-w-2xl w-full">
            {STEPS.map((step, idx) => {
          ...
          return (
            <div
              ...
              className={`zoom-card-transition transform-gpu relative flex flex-col justify-between p-6 sm:p-7 rounded-3xl cursor-pointer select-none border-2 ${
                isFocused
                  ? `scale-[1.02] sm:scale-[1.04] -translate-y-2 z-20 ${step.borderGlow} bg-gradient-to-b from-purple-950/40 to-slate-950/80 border-purple-500/80`
                  : 'scale-100 z-10 opacity-80 hover:opacity-100 bg-white/[0.02] border-white/[0.08] hover:border-white/20'
              } backdrop-blur-xl`}
            >
              ...
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-[10px] font-mono tracking-widest text-purple-300 uppercase block">
                      BƯỚC {step.number}
                    </span>
                ...
                {isFocused && (
                  <span className="flex items-center gap-1 text-[10px] font-bold text-pink-400 bg-pink-950/40 px-2 py-0.5 rounded-full border border-pink-500/30 animate-pulse">
              ...
              <div className="flex-1 flex flex-col mb-6">
                ...
                <p className="text-xs font-medium text-purple-300/80 mb-3.5">
                  {step.subtitle}
                </p>
                ...
                <div className="space-y-2 mt-auto pt-4 border-t border-white/10">
                  {step.perks.map((perk, perkIdx) => (
                    <div key={perkIdx} className="flex items-center gap-2 text-xs text-white/80">
                      <CheckCircle2 className={`w-3.5 h-3.5 shrink-0 ${isFocused ? 'text-green-400' : 'text-purple-400'}`} />
                      <span>{perk}</span>
                    </div>
                  ))}
                </div>
              ...
              <div className="pt-2 flex items-center justify-between text-xs font-semibold">
                <span className={`transition-colors ${isFocused ? 'text-white' : 'text-white/50'}`}>
                  {idx === 0 && 'Role chọn tự do'}
                  {idx === 1 && 'Học 24/7'}
                  {idx === 2 && 'Sinh tồn 2 ❤️❤️'}
                  {idx === 3 && 'Lên Rank Discord'}
                </span>
                <ArrowRight className={`w-4 h-4 transition-transform duration-200 ${isFocused ? 'translate-x-1 text-pink-400' : 'text-white/40'}`} />
              </div>
